...
- **Some decks are excluded from the pool.** The Multiplayer mod's own decks (Gradient, Violet, Cocktail, ...) are PvP-special: they install global `Card` hooks that run during normal gameplay and crash on cards they do not expect, so Deck Fixer leaves them out of the list entirely. Select them directly in a lobby instead.
